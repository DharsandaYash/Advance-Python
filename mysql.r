...
def init():
    cnn = sq.connect(host="localhost", user="root", password="", database="my")
    cursor = cnn.cursor();
    cursor.execute("create table if not exists My_Record(id int primary key auto_increment,name varchar(50),price int)")
    cursor.close();
    cnn.close();
init();
...
def delete_data():
    ...
    if(a>0):
        ...
        if(msg):
            ...
            cnn = sq.connect(host="localhost", user="root", password="", database="my");
            cursor = cnn.cursor();
            cursor.execute("delete from My_Record where id=(%s)",(v1,));
            cnn.commit();
            cursor.close()
            cnn.close();
...
def update_data():
    ...
    if (a>0):
        ...
        def up_data():
            ...
            cnn = sq.connect(host="localhost", user="root", password="", database="my");
            cursor = cnn.cursor();
            cursor.execute("update My_Record set name=(%s),price=(%s) where id=(%s)",(name,price,v[0]));
            cnn.commit();
